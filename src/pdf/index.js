...
function setHandler(pdfDocument, dataProvider) {
	let handler = {};
...
async function getPageChars(pdfDocument, dataProvider, pageIndex) {
	setHandler(pdfDocument, dataProvider);
	return pdfDocument.module.getPageChars(pageIndex);
}

async function getPageCharsObjects(pdfDocument, dataProvider, pageIndex) {
	setHandler(pdfDocument, dataProvider);
	return pdfDocument.module.getPageCharsObjects(pageIndex);
}

async function getPageLabels(pdfDocument, dataProvider) {
	setHandler(pdfDocument, dataProvider);
	return pdfDocument.module.getPageLabels();
}

async function writeAnnotations(buf, annotations, password, dataProvider) {
...
async function importAnnotations(buf, existingAnnotations, password, transfer, dataProvider) {
	...
	let pageLabels = await getPageLabels(pdfDocument, dataProvider);
	...
	for (let annotation of imported) {
		...
		let chars = await getPageChars(pdfDocument, dataProvider, pageIndex);
...
async function getPdfManager(arrayBuffer, password) {
	// Ensure we have a properly aligned ArrayBuffer (Node.js Buffers from
	// small readFileSync calls may share a pool ArrayBuffer with a non-zero byteOffset)
	if (arrayBuffer.byteOffset !== undefined && arrayBuffer.byteOffset !== 0) {
...
export async function getPages(buf, password, dataProvider) {
...
async function getOutline(buf, password, dataProvider) {
	let pdfManager = await getPdfManager(buf);
...
async function getProcessedData(buf, password, dataProvider) {
	let pdfManager = await getPdfManager(buf);
	setHandler(pdfManager.pdfDocument, dataProvider);
 ...
 * @param      {Function} dataProvider           (path) => Promise<ArrayBuffer>
 * @param      {Object}   [arg3={}]              Additional configuration
 * @param      {boolean}  [arg3.keepText=false]  Whether to keep text from
 ...
 */
async function processAnnotations(annotations, pdf, dataProvider, { keepText = false, fixTiny = false } = {}) {
	let pageHeight;
...
async function importCitaviAnnotations(buf, citaviAnnotations, password, dataProvider) {
	...
	await processAnnotations(annotations, pdf, dataProvider, { keepText: true });
	return annotations;
}

async function importMendeleyAnnotations(buf, mendeleyAnnotations, password, dataProvider) {
	let pdf = new PDFAssembler();
	await pdf.init(buf, password);
	let pdfDocument = pdf.pdfManager.pdfDocument;
	...
	await processAnnotations(annotations, pdf, dataProvider, { fixTiny: true });
	return annotations;
}
