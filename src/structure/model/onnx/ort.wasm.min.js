...
var H = (r, t) => () => (r && (t = r(r = 0)), t);
var rt = (r, t) => () => (t || r((t = { exports: {} }).exports, t), t.exports), gt = (r, t) => {
	for (var n in t) tr(r, n, { get: t[n], enumerable: !0 })
}, Ca = (r, t, n, f) => {
	if (t && typeof t == "object" || typeof t == "function") {
		for (let i of Sa(t)) !Ma.call(r, i) && i !== n && tr(r, i, {
			get: () => t[i],
			enumerable: !(f = Aa(t, i)) || f.enumerable
		});
	}
	return r
};
var nt = r => Ca(tr({}, "__esModule", { value: !0 }), r);
var Rt, Ve, yt, Pa, Dt, Ut = H(() => {
	"use strict";
	Rt = new Map, Ve = [], yt = (r, t, n) => {
		if (t && typeof t.init == "function" && typeof t.createInferenceSessionHandler == "function") {
			let f = Rt.get(r);
			if (f === void 0) {
				Rt.set(r, { backend: t, priority: n });
			}
			else {
				if (f.priority > n) {
					return;
				}
				if (f.priority === n && f.backend !== t) {
		...
		}
		{
			let n = !!t.initPromise;
			try {
				return n || (t.initPromise = t.backend.init(r)), await t.initPromise, t.initialized = !0, t.backend
			}
			catch (f) {
				return n || (t.error = `${f}`, t.aborted = !0), t.error
			}
			finally {
		...
		let t = r.executionProviders || [], n = t.map(h => typeof h == "string" ? h : h.name),
			f = n.length === 0 ? Ve : n, i, m = [], c = new Set;
		for (let h of f) {
			let g = await Pa(h);
			typeof g == "string" ? m.push({ name: h, err: g }) : (i || (i = g), i === g && c.add(h))
		}
		if (!i) {
			throw new Error(`no available backend found. ERR: ${m.map(h => `[${h.name}] ${h.err}`).join(", ")}`);
		}
		for (let {
			name: h,
			err: g
		} of m) n.includes(h) && console.warn(`removing requested execution provider "${h}" from session options because it is not available: ${g}`);
...
var en, _e, rr = H(() => {
	...
	Kr();
	en = "warning", _e = {
		wasm: {}, webgl: {}, webgpu: {}, versions: { common: Qr }, set logLevel(r) {
			if (r !== void 0) {
				if (typeof r != "string" || ["verbose", "info", "warning", "error", "fatal"].indexOf(r) === -1) {
	...
	Object.defineProperty(_e, "logLevel", { enumerable: !0 })
});
var j, tn = H(() => {
	"use strict";
	rr();
	...
	rn = (r, t) => {
		...
		if (f != null) {
			...
			for (let U = 0; U < m; U++) for (let D = 0; D < i; D++) {
				let C = (r.data[v++] - g[0]) * h[0], F = (r.data[O++] - g[1]) * h[1], W = (r.data[M++] - g[2]) * h[2],
					Z = P === -1 ? 255 : (r.data[P++] - g[3]) * h[3];
		...
		if (n != null) {
			...
			let v = m * i;
			if (t !== void 0 && (t.format !== void 0 && c === 4 && t.format !== "RGBA" || c === 3 && t.format !== "RGB" && t.format !== "BGR")) {
...
var nr, on, sn, un, fn, ln = H(() => {
	...
	nr = (r, t) => {
		...
		let s = t.format !== void 0 ? t.format : "RGBA",
			h = t.tensorFormat !== void 0 && t.tensorFormat !== void 0 ? t.tensorFormat : "RGB", g = n * f,
			_ = h === "RGBA" ? new Float32Array(g * 4) : new Float32Array(g * 3), v = 4, O = 0, M = 1, P = 2, U = 3,
			D = 0, C = g, F = g * 2, W = -1;
		...
		let n = typeof HTMLImageElement < "u" && r instanceof HTMLImageElement,
			f = typeof ImageData < "u" && r instanceof ImageData,
			i = typeof ImageBitmap < "u" && r instanceof ImageBitmap, m = typeof r == "string", c, s = t ?? {},
			h = () => {
		...
		if (n) {
			...
			if (t !== void 0 && t.resizedWidth !== void 0 && t.resizedHeight !== void 0 ? (_ = t.resizedHeight, v = t.resizedWidth) : (_ = r.height, v = r.width), t !== void 0 && (s = t), s.format = "RGBA", s.height = _, s.width = v, t !== void 0) {
				...
				let M = g(O);
				if (M != null) {
					M.putImageData(r, 0, 0), c = M.getImageData(0, 0, v, _).data;
				}
				else {
					throw new Error("Can not access image data")
				}
...
var Xe, wt, cn, dn, pn = H(() => {
	"use strict";
	Xe = new Map([["float32", Float32Array], ["uint8", Uint8Array], ["int8", Int8Array], ["uint16", Uint16Array], ["int16", Int16Array], ["int32", Int32Array], ["bool", Uint8Array], ["float64", Float64Array], ["uint32", Uint32Array]]), wt = new Map([[Float32Array, "float32"], [Uint8Array, "uint8"], [Int8Array, "int8"], [Uint16Array, "uint16"], [Int16Array, "int16"], [Int32Array, "int32"], [Float64Array, "float64"], [Uint32Array, "uint32"]]), cn = !1, dn = () => {
		if (!cn) {
			cn = !0;
			let r = typeof BigInt64Array < "u" && BigInt64Array.from,
				t = typeof BigUint64Array < "u" && BigUint64Array.from,
				n = typeof Float16Array < "u" && Float16Array.from;
	...
	mn = r => {
		...
		for (let n = 0; n < r.length; n++) {
			...
			if (typeof f != "number" || !Number.isSafeInteger(f)) {
				throw new TypeError(`dims[${n}] must be an integer, got: ${f}`);
			}
			if (f < 0) {
				throw new RangeError(`dims[${n}] must be a non-negative integer, got: ${f}`);
			}
			t *= f
		}
		return t
	...
	ye = class {
		constructor(t, n, f) {
			...
			if (typeof t == "object" && "location" in t) {
				switch (this.dataLocation = t.location, i = t.type, m = t.dims, t.location) {
					case"cpu-pinned": {
						let s = Xe.get(i);
						if (!s) {
					...
					case"gpu-buffer": {
						if (i !== "float32" && i !== "float16" && i !== "int32" && i !== "int64" && i !== "uint32" && i !== "uint8" && i !== "bool") {
				...
				if (typeof t == "string") {
					if (i = t, h = f, t === "string") {
						...
						if (Array.isArray(n)) {
							...
							throw new TypeError(`A ${i} tensor's data must be type of ${g}`)
						}
					}
				}
			...
			let c = mn(m);
			if (this.cpuData && c !== this.cpuData.length) {
...
var Bt, bn = H(() => {
	...
	Bt = class r {
		...
		async run(t, n, f) {
				...
				if (Array.isArray(n)) {
					...
					for (let g of n) {
						...
						if (this.outputNames.indexOf(g) === -1) {
							throw new RangeError(`'fetches' contains invalid output name: ${g}.`);
						}
						i[g] = null
					}
					...
					let g = !1, _ = Object.getOwnPropertyNames(n);
					for (let v of this.outputNames) if (_.indexOf(v) !== -1) {
						let O = n[v];
						(O === null || O instanceof me) && (g = !0, c = !1, i[v] = O)
					}
					if (g) {
						if (typeof f == "object" && f !== null) {
							m = f;
			...
			for (let g of this.inputNames) if (typeof t[g] > "u") {
				throw new Error(`input '${g}' is missing in 'feeds'.`);
			}
			if (c) {
				for (let g of this.outputNames) i[g] = null;
			}
		...
		static async create(t, n, f, i) {
			...
			if (typeof t == "string") {
				...
				if (typeof n == "object" && n !== null) {
					...
					if (v = n, !Number.isSafeInteger(v)) {
						throw new RangeError("'byteOffset' must be an integer.");
					}
					if (v < 0 || v >= _.byteLength) {
						throw new RangeError(`'byteOffset' is out of range [0, ${_.byteLength}).`);
					}
					if (O = t.byteLength - v, typeof f == "number") {
						if (O = f, !Number.isSafeInteger(O)) {
							throw new RangeError("'byteLength' must be an integer.");
						}
						if (O <= 0 || v + O > _.byteLength) {
							throw new RangeError(`'byteLength' is out of range (0, ${_.byteLength - v}].`);
						}
...
var Ra, Lt, An = H(() => {
	...
	Ra = "Training backend could not be resolved. Make sure you're using the correct configuration & WebAssembly files.", Lt = class r {
		...
		typeNarrowingForRunStep(t, n, f, i, m) {
			let c = {}, s = {};
			if (typeof f != "object" || f === null || f instanceof me || Array.isArray(f)) {
				throw new TypeError("'feeds' must be an object that use input names as keys and OnnxValue as corresponding values.");
			}
			let h = !0;
			if (typeof i == "object") {
				...
				if (Array.isArray(i)) {
					...
					let g = !1, _ = Object.getOwnPropertyNames(i);
					for (let v of n) if (_.indexOf(v) !== -1) {
						let O = i[v];
						(O === null || O instanceof me) && (g = !0, h = !1, c[v] = O)
					}
					if (g) {
						if (typeof m == "object" && m !== null) {
							s = m;
			...
			for (let g of t) if (typeof f[g] > "u") {
				throw new Error(`input '${g}' is missing in 'feeds'.`);
			}
			if (h) {
				for (let g of n) c[g] = null;
			}
			return [c, s]
		}
		...
		async runTrainStep(t, n, f) {
			let [i, m] = this.typeNarrowingForRunStep(this.trainingInputNames, this.trainingOutputNames, t, n, f),
				c = await this.handler.runTrainStep(t, i, m);
		...
		async runEvalStep(t, n, f) {
			if (this.hasEvalModel) {
				let [i, m] = this.typeNarrowingForRunStep(this.evalInputNames, this.evalOutputNames, t, n, f),
					c = await this.handler.runEvalStep(t, i, m);
				return this.convertHandlerReturnTypeToMapOfTensors(c)
			}
			else {
		...
		async loadParametersBuffer(t, n = !0) {
	...
	var Pn = (() => {
		...
		return typeof __filename < "u" && (r ||= __filename), function (t = {}) {
			var n = t, f, i, m = new Promise((a, l) => {
					f = a, i = l
				}), c = Object.assign({}, n), s = "./this.program", h = typeof window == "object",
				g = typeof importScripts == "function",
				_ = typeof process == "object1" && typeof process.versions == "object" && typeof process.versions.node == "string",
				v = "", O, M, P;
			if (_) {
				var U = (sr(), nt(ir)), D = (fr(), nt(ur));
				v = g ? D.dirname(v) + "/" : __dirname + "/", O = (a, l) => (a = ge(a) ? new URL(a) : D.normalize(a), U.readFileSync(a, l ? void 0 : "utf8")), P = a => (a = O(a, !0), a.buffer || (a = new Uint8Array(a)), a), M = (a, l, d, b = !0) => {
				...
				(h || g) && (g ? v = self.location.href : typeof document < "u" && document.currentScript && (v = document.currentScript.src), r && (v = r), v.startsWith("blob:") ? v = "" : v = v.substr(0, v.replace(/[?#].*/, "").lastIndexOf("/") + 1), O = a => {
					var l = new XMLHttpRequest;
					return l.open("GET", a, !1), l.send(null), l.responseText
				}, g && (P = a => {
					var l = new XMLHttpRequest;
			...
			function ce(a) {
				throw a = "Aborted(" + a + ")", F(a), Z = !0, a = new WebAssembly.RuntimeError(a + ". Build with -sASSERTIONS for more info."), i(a), a
			}
			...
			function ut(a) {
				if (h || g) {
					if (typeof fetch == "function" && !ge(a)) {
						return fetch(a, { credentials: "same-origin" }).then(l => {
							if (!l.ok) {
			...
			var Me, Re = {
				798088: (a, l, d, b) => {
					...
					if (a = de(a >>> 0), a.startsWith("./") && (a = a.substring(2)), a = n.ya.get(a), !a) {
						return 2;
					}
					if (l >>>= 0, d >>>= 0, l + d > a.byteLength) {
						return 3;
					}
					try {
			...
			var be = 0, Ge = 0, oe = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, ie = (a, l, d) => {
					...
					var b = l + d;
					for (d = l; a[d] && !(d >= b);) ++d;
					if (16 < d - l && a.buffer && oe) {
						return oe.decode(a.subarray(l, d));
					}
					for (b = ""; l < d;) {
						var A = a[l++];
						if (A & 128) {
							var R = a[l++] & 63;
							if ((A & 224) == 192) {
								b += String.fromCharCode((A & 31) << 6 | R);
							}
							else {
								var N = a[l++] & 63;
					...
					return l[d >>> 0] = 0, d - A
				}, We = a => a % 4 === 0 && (a % 100 !== 0 || a % 400 === 0),
				ft = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335],
				Et = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334], De = [], et = {}, je = () => {
					if (!ze) {
						var a = {
							USER: "web_user",
							LOGNAME: "web_user",
					...
					return ze
				}, ze, _t = [null, [], []], Ie = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31],
			...
			function lt(a, l, d, b) {
				...
				function Ee(y) {
					...
					for (y = new Date(new Date(y.ta + 1900, 0, 1).getTime()); 0 < $;) {
						...
						if ($ > Ue - y.getDate()) {
							$ -= Ue - y.getDate() + 1, y.setDate(1), 11 > Y ? y.setMonth(Y + 1) : (y.setMonth(0), y.setFullYear(y.getFullYear() + 1));
						}
						else {
							y.setDate(y.getDate() + $);
							break
				...
				var mt = "Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" "),
					He = "January February March April May June July August September October November December".split(" ");
				fe = {
					"%a": y => mt[y.na].substring(0, 3),
					"%A": y => mt[y.na],
					"%b": y => He[y.va].substring(0, 3),
					"%B": y => He[y.va],
					"%C": y => R((y.ta + 1900) / 100 | 0, 2),
					"%d": y => R(y.xa, 2),
					"%e": y => A(y.xa, 2, " "),
					"%g": y => Ee(y).toString().substring(2),
					"%G": Ee,
					"%H": y => R(y.ua, 2),
					"%I": y => (y = y.ua, y == 0 ? y = 12 : 12 < y && (y -= 12), R(y, 2)),
					"%j": y => {
					...
					"%n": () => `
`,
					"%p": y => 0 <= y.ua && 12 > y.ua ? "AM" : "PM",
					"%S": y => R(y.Ba, 2),
					"%t": () => "	",
					"%u": y => y.na || 7,
					"%U": y => R(Math.floor((y.sa + 7 - y.na) / 7), 2),
					"%V": y => {
						var $ = Math.floor((y.sa + 7 - (y.na + 6) % 7) / 7);
						if (2 >= (y.na + 371 - y.sa - 2) % 7 && $++, $) {
							$ == 53 && (Y = (y.na + 371 - y.sa) % 7, Y == 4 || Y == 3 && We(y.ta) || ($ = 1));
						}
						else {
							$ = 52;
					...
					"%w": y => y.na,
					"%W": y => R(Math.floor((y.sa + 7 - (y.na + 6) % 7) / 7), 2),
					"%y": y => (y.ta + 1900).toString().substring(2),
					"%Y": y => y.ta + 1900,
					"%z": y => {
						y = y.za;
						var $ = 0 <= y;
			...
			var ct = {
				a: function (a, l, d) {
					...
					a >>>= 0;
					var l = new Date(x[a + 20 >>> 2 >>> 0] + 1900, x[a + 16 >>> 2 >>> 0], x[a + 12 >>> 2 >>> 0], x[a + 8 >>> 2 >>> 0], x[a + 4 >>> 2 >>> 0], x[a >>> 2 >>> 0], 0),
						d = x[a + 32 >>> 2 >>> 0], b = l.getTimezoneOffset(),
						A = new Date(l.getFullYear(), 6, 1).getTimezoneOffset(),
						R = new Date(l.getFullYear(), 0, 1).getTimezoneOffset(), N = Math.min(R, A);
					...
					d >>>= 0, b >>>= 0;
					var A = new Date().getFullYear(), R = new Date(A, 0, 1), N = new Date(A, 6, 1);
					A = R.getTimezoneOffset();
					var ae = N.getTimezoneOffset();
					Q[a >>> 0 >>> 2 >>> 0] = 60 * Math.max(A, ae), x[l >>> 0 >>> 2 >>> 0] = +(A != ae), a = Ee => Ee.toLocaleTimeString(void 0, {
						hour12: !1,
						timeZoneName: "short"
					}).split(" ")[1], R = a(R), N = a(N), ae < A ? (xe(R, re, d, 17), xe(N, re, b, 17)) : (xe(R, re, b, 17), xe(N, re, d, 17))
				}, d: () => {
					ce("")
				}, B: function (a, l, d) {
			...
			function St() {
				...
				a = Object.assign({}, a);
				var l = d => b => d(b) >>> 0;
				return a.la = l(a.la), a.qa = l(a.qa), a.ra = (d => () => d() >>> 0)(a.ra), a
			}
			...
			function pt() {
				if (!(0 < Ae)) {
					...
					if (!(0 < Ae || Be || (Be = !0, n.calledRun = !0, Z))) {
	...
	var Bn = (() => {
		...
		return typeof __filename < "u" && (r ||= __filename), function (t = {}) {
			...
			var s = t, h, g, _ = new Promise((e, o) => {
					...
					throw o
				}, P = typeof window == "object", U = typeof importScripts == "function",
				D = typeof process == "object" && typeof process.versions == "object" && typeof process.versions.node == "string",
				C = s.ENVIRONMENT_IS_PTHREAD || !1, F = "";
			...
			if (D) {
				...
				F = U ? Q.dirname(F) + "/" : __dirname + "/", Z = (e, o) => (e = de(e) ? new URL(e) : Q.normalize(e), x.readFileSync(e, o ? void 0 : "utf8")), re = e => (e = Z(e, !0), e.buffer || (e = new Uint8Array(e)), e), te = (e, o, u, p = !0) => {
				...
				(P || U) && (U ? F = self.location.href : typeof document < "u" && document.currentScript && (F = document.currentScript.src), typeof r < "u" && r && (F = r), F.startsWith("blob:") ? F = "" : F = F.substr(0, F.replace(/[?#].*/, "").lastIndexOf("/") + 1), D || (Z = e => {
					var o = new XMLHttpRequest;
					return o.open("GET", e, !1), o.send(null), o.responseText
				}, U && (re = e => {
					var o = new XMLHttpRequest;
...
`));
			...
			var K, Ae, we = !1, L, ce, ke, ge, Oe, ue;
			...
			function oe(e) {
				throw e = "Aborted(" + e + ")", se(e), we = !0, L = 1, e = new WebAssembly.RuntimeError(e + ". Build with -sASSERTIONS for more info."), g(e), e
			}
			...
			function We(e) {
				if (P || U) {
					if (typeof fetch == "function" && !de(e)) {
						return fetch(e, { credentials: "same-origin" }).then(o => {
							if (!o.ok) {
			...
			var De, et = {
				799444: (e, o, u, p) => {
					...
					if (e = ae(e >>> 0), e.startsWith("./") && (e = e.substring(2)), e = s.bb.get(e), !e) {
						return 2;
					}
					if (o >>>= 0, u >>>= 0, p >>>= 0, o + u > e.byteLength) {
						return 3;
					}
					try {
			...
			function lt(e) {
				...
				L = e, 0 < Ie || (I.hb(), s.onExit?.(e), we = !0), M(e, new je(e))
			}
				...
				Oa: [], Pa: [], ab: [], La: {}, Va() {
					...
					e.onmessage = E => {
						...
						if (E.targetThread && E.targetThread != Ct()) {
							...
							S === "checkMailbox" ? Mt() : S === "spawnThread" ? _t(E) : S === "cleanupThread" ? I.Ya(I.La[E.thread]) : S === "killThread" ? (E = E.thread, S = I.La[E], delete I.La[E], ze(S), Zt(E), I.Pa.splice(I.Pa.indexOf(S), 1), S.Na = 0) : S === "cancelThread" ? I.La[E.thread].postMessage({ cmd: "cancel" }) : S === "loaded" ? (e.loaded = !0, D && !e.Na && e.unref(), o(e)) : S === "alert" ? alert(`Thread ${E.threadId}: ${E.text}`) : E.target === "setimmediate" ? e.postMessage(E) : S === "callHandler" ? s[E.handler](...E.args) : S && se(`worker sent an unknown command ${S}`)
						}
					}, e.onerror = E => {
					...
					e.postMessage({
						cmd: "load",
						handlers: u,
						urlOrBlob: s.mainScriptUrlOrBlob || r,
						wasmMemory: K,
						wasmModule: Ae
					})
				})
			...
			function A(e, o, u, p) {
				if (e >>>= 0, o >>>= 0, u >>>= 0, p >>>= 0, typeof SharedArrayBuffer > "u") {
					return se("Current environment does not support SharedArrayBuffer, pthreads are not available!"), 6;
				}
				var T = [];
				return C && T.length === 0 ? b(e, o, u, p) : (e = {
					gb: u,
					Na: e,
					cb: p,
					mb: T
				}, C ? (e.ob = "spawnThread", postMessage(e, T), 0) : _t(e))
			}
			...
			var R = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, N = (e, o, u) => {
				...
				var p = o + u;
				for (u = o; e[u] && !(u >= p);) ++u;
				if (16 < u - o && e.buffer && R) {
					return R.decode(e.buffer instanceof SharedArrayBuffer ? e.slice(o, u) : e.subarray(o, u));
				}
				for (p = ""; o < u;) {
					var T = e[o++];
					if (T & 128) {
						var E = e[o++] & 63;
						if ((T & 224) == 192) {
							p += String.fromCharCode((T & 31) << 6 | E);
						}
						else {
							var S = e[o++] & 63;
			...
			function Er(e, o, u, p) {
				if (C) {
					return z(10, 1, e, o, u, p)
				}
			}

			function _r(e, o, u, p) {
				if (C) {
					return z(11, 1, e, o, u, p)
				}
			}

			function Tr(e, o, u, p) {
				if (C) {
					return z(12, 1, e, o, u, p)
				}
			}
			...
			s.checkMailbox = Mt;
			var qt = [], tt = e => e % 4 === 0 && (e % 100 !== 0 || e % 400 === 0),
			...
			var Nr = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31],
				Wr = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
			...
			function Hr(e, o, u, p) {
				...
				function Fe(w) {
					...
					for (w = new Date(new Date(w.Ra + 1900, 0, 1).getTime()); 0 < G;) {
						...
						if (G > qe - w.getDate()) {
							G -= qe - w.getDate() + 1, w.setDate(1), 11 > J ? w.setMonth(J + 1) : (w.setMonth(0), w.setFullYear(w.getFullYear() + 1));
						}
						else {
				...
				var Vr = "Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" "),
					Xr = "January February March April May June July August September October November December".split(" ");
				pe = {
					"%a": w => Vr[w.Ma].substring(0, 3),
					"%A": w => Vr[w.Ma],
					"%b": w => Xr[w.Ta].substring(0, 3),
					"%B": w => Xr[w.Ta],
					"%C": w => E((w.Ra + 1900) / 100 | 0, 2),
					"%d": w => E(w.Wa, 2),
					"%e": w => T(w.Wa, 2, " "),
					"%g": w => Fe(w).toString().substring(2),
					"%G": Fe,
					"%H": w => E(w.Sa, 2),
					"%I": w => (w = w.Sa, w == 0 ? w = 12 : 12 < w && (w -= 12), E(w, 2)),
					"%j": w => {
					...
					"%n": () => `
`,
					"%p": w => 0 <= w.Sa && 12 > w.Sa ? "AM" : "PM",
					"%S": w => E(w.kb, 2),
					"%t": () => "	",
					"%u": w => w.Ma || 7,
					"%U": w => E(Math.floor((w.Qa + 7 - w.Ma) / 7), 2),
					"%V": w => {
						var G = Math.floor((w.Qa + 7 - (w.Ma + 6) % 7) / 7);
						if (2 >= (w.Ma + 371 - w.Qa - 2) % 7 && G++, G) {
							G == 53 && (J = (w.Ma + 371 - w.Qa) % 7, J == 4 || J == 3 && tt(w.Ra) || (G = 1));
						}
						else {
							G = 52;
					...
					"%w": w => w.Ma,
					"%W": w => E(Math.floor((w.Qa + 7 - (w.Ma + 6) % 7) / 7), 2),
					"%y": w => (w.Ra + 1900).toString().substring(2),
					"%Y": w => w.Ra + 1900,
					"%z": w => {
						w = w.ib;
						var G = 0 <= w;
			...
			I.Va();
			var Ea = [lt, St, b, Ee, fe, y, $, Y, Ue, ht, Er, _r, Tr, Ar, Sr, Mr, xr, Rr, Ur, Fr, Ir, Br, Lr, kr],
				_a = {
					b: function (e, o, u) {
						throw e >>>= 0, new a(e).Va(o >>> 0, u >>> 0), l = e, d++, l
					},
					L: function (e) {
						$r(e >>> 0, !U, 1, !P, 131072, !1), I.$a()
					},
					j: function (e) {
					...
					p: () => 1,
					z: function (e, o) {
						e >>>= 0, e == o >>> 0 ? setTimeout(Mt) : C ? postMessage({
							targetThread: e,
							cmd: "checkMailbox"
						}) : (e = I.La[e]) && e.postMessage({ cmd: "checkMailbox" })
					},
					...
					u: function (e) {
						e >>>= 0;
						var o = new Date(i()[e + 20 >>> 2 >>> 0] + 1900, i()[e + 16 >>> 2 >>> 0], i()[e + 12 >>> 2 >>> 0], i()[e + 8 >>> 2 >>> 0], i()[e + 4 >>> 2 >>> 0], i()[e >>> 2 >>> 0], 0),
							u = i()[e + 32 >>> 2 >>> 0], p = o.getTimezoneOffset(),
							T = new Date(o.getFullYear(), 6, 1).getTimezoneOffset(),
							E = new Date(o.getFullYear(), 0, 1).getTimezoneOffset(), S = Math.min(E, T);
					...
					y: function (e, o, u, p) {
						e >>>= 0, o >>>= 0, u >>>= 0, p >>>= 0;
						var T = new Date().getFullYear(), E = new Date(T, 0, 1), S = new Date(T, 6, 1);
						T = E.getTimezoneOffset();
						var X = S.getTimezoneOffset(), Fe = Math.max(T, X);
						m()[e >>> 2 >>> 0] = 60 * Fe, i()[o >>> 2 >>> 0] = +(T != X), e = pe => pe.toLocaleTimeString(void 0, {
							hour12: !1,
							timeZoneName: "short"
						}).split(" ")[1], E = e(E), S = e(S), X < T ? (He(E, u, 17), He(S, p, 17)) : (He(E, p, 17), He(S, u, 17))
					},
					c: () => {
						oe("")
					},
					...
					f: () => D ? (In(), nt(Fn)).cpus().length : navigator.hardwareConcurrency,
					x: function (e) {
						e >>>= 0;
						var o = f().length;
						if (e <= o || 4294901760 < e) {
			...
			var Gr = (e, o, u, p, T) => (Gr = B.Ca)(e, o, u, p, T), Zt = e => (Zt = B.Da)(e),
				Qt = s.__emscripten_thread_exit = e => (Qt = s.__emscripten_thread_exit = B.Ea)(e),
				jr = () => (jr = B.Fa)(), zr = e => (zr = B.Ga)(e), Yr = (e, o) => (Yr = B.Ha)(e, o),
				Pt = e => (Pt = B.Ia)(e), Kt = e => (Kt = B.Ja)(e), er = () => (er = B.Ka)();
			...
			function qr() {
				if (!(0 < Re)) {
					if (C) {
						h(s), C || Ye(Ne), startWorker(s);
					}
					else {
						if (s.preRun) {
							for (typeof s.preRun == "function" && (s.preRun = [s.preRun]); s.preRun.length;) Se.unshift(s.preRun.shift());
						}
						Ye(Se), 0 < Re || xt || (xt = !0, s.calledRun = !0, we || (C || Ye(Ne), h(s), C || Ye(Me)))
					}
				}
			}
...
var Hn, ka, dr, pr, kt, Wn, Na, Wa, Ha, $n, ee, at = H(() => {
	"use strict";
	Hn = Rn();
	ka = kn(), pr = !1, kt = !1, Wn = !1, Na = r => {
		if (r === 1) {
			return !1;
		}
		if (typeof SharedArrayBuffer > "u") {
			return typeof self < "u" && !self.crossOriginIsolated && console.warn("env.wasm.numThreads is set to " + r + ", but this will not work unless you enable crossOriginIsolated mode. See https://web.dev/cross-origin-isolation-guide/ for more info."), !1;
		}
		...
		kt = !0;
		let t = r.initTimeout, n = r.numThreads, f = r.simd, i = Na(n), m = f && Wa(), c = r.wasmPaths,
			s = typeof c == "string" ? c : void 0, h = Ha(m, i), g = typeof c == "object" ? c[h] : void 0, _ = !1,
			v = [];
		if (t > 0 && v.push(new Promise(O => {
			...
			let P = i ? ka : Hn,
				U = { locateFile: (D, C) => i && D.endsWith(".worker.js") && typeof Blob < "u" ? URL.createObjectURL(new Blob([Nn()], { type: "text/javascript" })) : D.endsWith(".wasm") ? g || (s ?? C) + h : C + D };
			if (i) {
				if (U.numThreads = n, typeof Blob > "u") {
					U.mainScriptUrlOrBlob = (void 0)(__dirname, "ort-wasm-threaded.js");
				}
				else {
	...
	q = (r, t) => {
		...
		Object.entries(r).forEach(([i, m]) => {
			...
			if (typeof m == "object") {
				...
				throw new Error(`Can't handle extra config type: ${typeof m}`)
			}
		})
	}, V = r => {
...
var Gn, jn = H(() => {
	...
	Gn = r => {
		let t = ee(), n = 0, f = [], i = r || {};
		try {
			if (r?.logSeverityLevel === void 0) {
				i.logSeverityLevel = 2;
			}
			else if (typeof r.logSeverityLevel != "number" || !Number.isInteger(r.logSeverityLevel) || r.logSeverityLevel < 0 || r.logSeverityLevel > 4) {
				throw new Error(`log serverity level is not valid: ${r.logSeverityLevel}`);
			}
			if (r?.logVerbosityLevel === void 0) {
				i.logVerbosityLevel = 0;
			}
			else if (typeof r.logVerbosityLevel != "number" || !Number.isInteger(r.logVerbosityLevel)) {
				throw new Error(`log verbosity level is not valid: ${r.logVerbosityLevel}`);
			}
			r?.terminate === void 0 && (i.terminate = !1);
			let m = 0;
			return r?.tag !== void 0 && (m = q(r.tag, f)), n = t._OrtCreateRunOptions(i.logSeverityLevel, i.logVerbosityLevel, !!i.terminate, m), n === 0 && V("Can't create run options."), r?.extra !== void 0 && bt(r.extra, "", new WeakSet, (c, s) => {
				let h = q(c, f), g = q(s, f);
...
var $a, Ga, ja, za, zn, Yn = H(() => {
	...
	$a = r => {
		...
		t.use_ort_model_bytes_directly || (t.use_ort_model_bytes_directly = "1"), r.executionProviders && r.executionProviders.some(n => (typeof n == "string" ? n : n.name) === "webgpu") && (r.enableMemPattern = !1)
	}, za = (r, t, n) => {
		for (let f of t) {
		...
		ja(i);
		try {
			let m = $a(i.graphOptimizationLevel ?? "all"), c = Ga(i.executionMode ?? "sequential"),
				s = typeof i.logId == "string" ? q(i.logId, f) : 0, h = i.logSeverityLevel ?? 2;
	...
	vt = async r => {
		if (typeof r == "string") {
			if (typeof process < "u" && process.versions && process.versions.node) {
				...
				{
					if (!t.body) {
						throw new Error(`failed to load external data file: ${r}, no response body.`);
					}
...
var Ya, Qn, Kn, ot, qa, wr, ea, ta, Zn, ra, na, aa, oa = H(() => {
	"use strict";
	jn();
	Yn();
	gr();
	...
	Ya = (r, t) => {
		...
		if (n === 0) {
			throw new Error(`Can't create a session. failed to allocate a buffer of size ${r.byteLength}.`);
		}
		...
		try {
			...
			let C = null;
			return ot.set(m, [m, g, _, C, M, !1]), [m, P, U]
		}
		catch (v) {
		...
		let t = ee(), n = ot.get(r);
		if (!n) {
			throw new Error(`cannot release session. invalid session id: ${r}`);
		}
		let [f, i, m, c, s] = n;
			...
			if (Array.isArray(P)) {
				...
				for (let D = 0; D < P.length; D++) {
					if (typeof P[D] != "string") {
						throw new TypeError(`tensor data at index ${D} is not a string`);
					}
					c.HEAPU32[U++] = q(P[D], n)
				}
			}
			else {
				v = P.byteLength, _ = c._malloc(v), n.push(_), c.HEAPU8.set(new Uint8Array(P.buffer, P.byteOffset, v), _)
			}
		}
		...
		let c = ee(), s = ot.get(r);
		if (!s) {
			throw new Error(`cannot run inference. invalid session id: ${r}`);
		}
		let h = s[0], g = s[1], _ = s[2], v = s[3], O = s[4], M = s[5], P = t.length, U = f.length, D = 0, C = [],
			F = [], W = [], Z = [], te = c.stackSave(), re = c.stackAlloc(P * 4), x = c.stackAlloc(P * 4),
			Q = c.stackAlloc(U * 4), Le = c.stackAlloc(U * 4);
		try {
			[D, C] = Gn(m);
			...
			for (let L = 0; L < U; L++) {
				...
				let ke = c.stackSave(), ge = c.stackAlloc(4 * 4), Oe = !1, ue, ne = 0;
				try {
					...
					if (ue === "string") {
						...
						Oe = !0, we.push([ue, Ce, {
							gpuBuffer: ie,
							download: c.jsepCreateDownloader(ie, be * de, ue),
							dispose: () => {
			...
			return v && !O && (c._OrtClearBoundOutputs(v.handle), ot.set(r, [h, g, _, v, O, !1])), we
		}
		finally {
...
var Ke, Te, Ot, $t, Gt, br, vr, it, st, Ja, Ht, sa, ua, fa, la, ca, da, pa, Or = H(() => {
	"use strict";
	Qe();
	oa();
	at();
	Ke = () => !!j.wasm.proxy && typeof document < "u", Ot = !1, $t = !1, Gt = !1, vr = new Map, it = (r, t) => {
		let n = vr.get(r);
		...
		switch (r.data.type) {
			case"init-wasm":
				Ot = !1, r.data.err ? (Gt = !0, br[1](r.data.err)) : ($t = !0, br[0]());
				break;
			case"init-ep":
			case"copy-from":
			case"create":
		...
		if (!$t) {
			...
			// Minimal wasmBinary support: if an in-memory binary is provided, expose it via blob URLs
			// so the loader will pick it up through wasmPaths mapping.
			if (j?.wasm?.wasmBinary && typeof URL !== "undefined" && typeof Blob !== "undefined") {
				const bin = j.wasm.wasmBinary instanceof Uint8Array ? j.wasm.wasmBinary : new Uint8Array(j.wasm.wasmBinary);
				const url = URL.createObjectURL(new Blob([bin], { type: "application/wasm" }));
				const map = {
					"ort-wasm.wasm": url,
				...
				j.wasm.wasmPaths = typeof j.wasm.wasmPaths === "object" ? { ...map, ...j.wasm.wasmPaths } : map;
			}

			if (Ot = !0, Ke()) {
				return j.wasm.wasmPaths === void 0 && Ht && Ht.indexOf("blob:") !== 0 && (j.wasm.wasmPaths = Ht.substr(0, +Ht.lastIndexOf("/") + 1)), new Promise((r, t) => {
					Te?.terminate();
		...
		if (Ke()) {
			...
			return st(), new Promise((c, s) => {
				it("run", [c, s]);
				let h = n,
					g = { type: "run", in: { sessionId: r, inputIndices: t, inputs: h, outputIndices: f, options: m } };
				Te.postMessage(g, aa(h))
			})
		}
		else {
			return ra(r, t, n, f, i, m)
		}
	}, pa = async r => {
		if (Ke()) {
			return st(), new Promise((t, n) => {
...
var ma, Za, jt, ha = H(() => {
	...
	ma = (r, t) => {
		...
		async run(t, n, f) {
			...
			let h = i.map((O, M) => ma(O, () => `input "${this.inputNames[m[M]]}"`)),
				g = c.map((O, M) => O ? ma(O, () => `output "${this.outputNames[s[M]]}"`) : null),
				_ = await da(this.sessionId, m, h, s, g, f), v = {};
...
var Qa, zt, ga = H(() => {
	...
	ha();
	Qa = () => {
		if ((typeof j.wasm.initTimeout != "number" || j.wasm.initTimeout < 0) && (j.wasm.initTimeout = 0), typeof j.wasm.simd != "boolean" && (j.wasm.simd = !0), typeof j.wasm.proxy != "boolean" && (j.wasm.proxy = !1), typeof j.wasm.trace != "boolean" && (j.wasm.trace = !1), typeof j.wasm.numThreads != "number" || !Number.isInteger(j.wasm.numThreads) || j.wasm.numThreads <= 0) {
...
Object.defineProperty(j.versions, "web", { value: Mn, enumerable: !0 });
export {
	xa as InferenceSession,
	yn as TRACE,
	Je as TRACE_FUNC_BEGIN,
	Ze as TRACE_FUNC_END,
	me as Tensor,
	Da as TrainingSession,
	Ni as default,
	j as env,
	yt as registerBackend
};
